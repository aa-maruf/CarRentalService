<?php
    ...
    if( $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ( !empty($name)) {
            ...
            $stmt = $conn->prepare("UPDATE Admin_Informations SET Admin_Name = ? WHERE Admin_ID = ?");
            $stmt->bind_param("si", $name, $_SESSION['Admin_ID']);
            $stmt->execute();

            if ($stmt->affected_rows > 0) {
                echo "";
            } else {
                $message = "No rows updated. Admin_ID may not exist or the value is already the same.";
            }
        }
        ...
        if (preg_match('/[A-Z\s]/', $email)) {
            ...
            $stmt = $conn->prepare("UPDATE Admin_Informations SET Admin_Email = ? WHERE Admin_ID = ?");
        ...
        // Updating Admin Contact Number
        if (!preg_match('/^(013|014|015|016|017|018|019)\d{8}$/', $num)) {
            $message = 'Invalid Contact Number';
        }
        else if (!empty($num)) {

            $stmt = $conn->prepare("UPDATE Admin_Informations SET Admin_Contact_Number = ? WHERE Admin_ID = ?");
            $stmt->bind_param("si", $num, $_SESSION['Admin_ID']);
            $stmt->execute();
        ...
        // Updating Admin Emergency Contact Number
        if (!preg_match('/^(013|014|015|016|017|018|019)\d{8}$/', $emer_num)) {
            $message = 'Invalid Contact Number';
        }
        else if (!empty($emer_num)) {

            $stmt = $conn->prepare("UPDATE Admin_Informations SET Admin_Emergency_Contact = ? WHERE Admin_ID = ?");
            $stmt->bind_param("si", $emer_num, $_SESSION['Admin_ID']);
            $stmt->execute();
        ...
        if ( !empty($gender)) {
            ...
            $stmt = $conn->prepare("UPDATE Admin_Informations SET Admin_Gender = ? WHERE Admin_ID = ?");
            $stmt->bind_param("si", $gender, $_SESSION['Admin_ID']);
            $stmt->execute();
        ...
        // Updating Admin Address
        if ( !empty($addr)) {

            $stmt = $conn->prepare("UPDATE Admin_Informations SET Admin_Address = ? WHERE Admin_ID = ?");
            $stmt->bind_param("si", $addr, $_SESSION['Admin_ID']);
            $stmt->execute();
